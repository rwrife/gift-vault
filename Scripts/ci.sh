...
artifact_root="${GIFTVAULT_ARTIFACT_DIR:-$repo_root/build/ci-artifacts}"
...
phase="iphone_only_prebuild"
python3 - <<'PYTHON'
import sys

text = open("GiftVault.xcodeproj/project.pbxproj", encoding="utf-8").read()
violations = [
    line.strip()
    for line in text.splitlines()
    if "TARGETED_DEVICE_FAMILY" in line and "TARGETED_DEVICE_FAMILY = 1;" not in line
]
count = text.count("TARGETED_DEVICE_FAMILY = 1;")
if violations:
    print("iPhone-only directive violated in project.pbxproj:", file=sys.stderr)
    for line in violations:
        print(f"  {line}", file=sys.stderr)
    raise SystemExit(1)
if count < 4:
    raise SystemExit(
        f"Expected TARGETED_DEVICE_FAMILY = 1 in >=4 app/UI-test build "
        f"configurations, found {count}"
    )
print(f"Pre-build iPhone-only guard: {count} configurations pin TARGETED_DEVICE_FAMILY = 1")
PYTHON
...
phase="domain_tests"
xcrun swift test \
  --package-path Packages/GiftVaultKit \
  2>&1 | tee "$artifact_dir/domain-tests.log"

phase="app_build"
xcodebuild build \
  -project GiftVault.xcodeproj \
  -scheme GiftVault \
  -configuration Debug \
...
app_plist="$derived_data/Build/Products/Debug-iphonesimulator/GiftVault.app/Info.plist"
if [[ ! -f "$app_plist" ]]; then
